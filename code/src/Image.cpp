...
bool Image::load(const std::string& path) {
    ...
    unsigned char* data = stbi_load(path.c_str(), &loadedWidth, &loadedHeight, &loadedChannels, 0);
...
bool Image::save(const std::string& path) const {
    ...
    int result = 0;
    if (extension == "png") {
        result = stbi_write_png(path.c_str(), width, height, channels, pixels.data(), width * channels);
    } else if (extension == "jpg" || extension == "jpeg") {
        result = stbi_write_jpg(path.c_str(), width, height, channels, pixels.data(), 90);
    } else if (extension == "bmp") {
        result = stbi_write_bmp(path.c_str(), width, height, channels, pixels.data());
    } else if (extension == "tga") {
        result = stbi_write_tga(path.c_str(), width, height, channels, pixels.data());
    } else {
        std::cerr << "Formato de arquivo não suportado: " << extension << std::endl;
        return false;
    }
...
unsigned char Image::getPixel(int x, int y, int c) const {
    if (x < 0 || x >= width || y < 0 || y >= height || c < 0 || c >= channels) {
        throw std::out_of_range("Coordenada de pixel fora dos limites");
    }
...
Image Image::toGrayscale() const {
    ...
    gray.height = height;
    gray.channels = 1;
    gray.pixels.resize(width * height);
...
Image Image::createColoredLabelImage(
    ...
    for (std::size_t i = 0; i < labels.size(); i++) {
        ...
        output.setPixel(x, y, 0, labelColors[label][0]);
        output.setPixel(x, y, 1, labelColors[label][1]);
        output.setPixel(x, y, 2, labelColors[label][2]);
    }
...
Image Image::createAvgColorLabelImage(const std::vector<int>& labels, const Image& original) {
    std::size_t expectedSize = static_cast<std::size_t>(original.width) * original.height;
    
    if (labels.size() != expectedSize) {
        std::cerr << "Erro interno: matriz de labels (" << labels.size() 
                  << ") nao bate com as dimensoes originais (" << expectedSize << ")." << std::endl;
        return Image();
    }
